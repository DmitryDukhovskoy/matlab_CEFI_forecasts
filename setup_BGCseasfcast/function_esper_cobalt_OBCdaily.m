...
for YR=YR1:YR2;
  ...
  for isegm=1:4;
    ...
    if ~isfield(DATA(kYR).segm(isegm), 'nx_segm') || ...
      ...
      % Variables and dimensions names:
      DATA(kYR).segm(isegm).segstr    = sprintf('_segment_%3.3i',isegm);
      DATA(kYR).segm(isegm).alk_var   = sprintf('alk%s',segstr);
      DATA(kYR).segm(isegm).dic_var   = sprintf('dic%s',segstr);
      DATA(kYR).segm(isegm).dzalk_var = sprintf('dz_alk%s',segstr);
      DATA(kYR).segm(isegm).dzdic_var = sprintf('dz_dic%s',segstr);
      DATA(kYR).segm(isegm).lat_var   = sprintf('lat%s',segstr);
      DATA(kYR).segm(isegm).lon_var   = sprintf('lon%s',segstr);
      DATA(kYR).segm(isegm).zdim_name = sprintf('nz%s',segstr);
      DATA(kYR).segm(isegm).ydim_name = sprintf('ny%s',segstr);
      DATA(kYR).segm(isegm).xdim_name = sprintf('nx%s',segstr);
    end
  ...
  if kYR == Nyrs_save
    ...
    for iyr = 1:Nyrs_save
      for isegm = 1:4
        segstr    = DATA(iyr).segm(isegm).segstr;
        alk_var   = DATA(iyr).segm(isegm).alk_var;
        ...
        fprintf('  %s: nx = %i, ny = %i,  time: %s to %s\n', segstr, nx, ny, dstr1, dstr2);
        fprintf('  alk_var=%s dic_var=%s dzalk_var=%s dzdic_var=%s\n',...
                   alk_var, dic_var, dzalk_var, dzdic_var);
        ...
        alkT = permute(alkT, [4 3 2 1]);
        dicT = reshape(dicT, nt, nz, ny, nx); 
        dicT = permute(dicT, [4 3 2 1]);
